...
Pada development, BFF menggunakan `NEXT_PUBLIC_API_URL`. Pada production atau
container, BFF memprioritaskan `INTERNAL_API_URL` agar request server-side tidak
bergantung pada URL browser.
...
2. Menjalankan format check, lint, typecheck, unit test, dan production build.
3. Build image dan push ke GHCR dengan tag commit SHA.
4. Membuat `PROD_APP_DIR` pada VPS.
5. Mengupload `compose.yaml` dan membuat `.env` production.
6. Pull image terbaru dan restart Compose.
7. Memastikan container web mencapai status `healthy`.
...
| `PROD_APP_DIR`     | Contoh: `/srv/apps/tracklume-web`                 |
...
Workflow tidak membutuhkan `compose.yaml` atau `.env` yang dibuat manual.
Keduanya disiapkan setiap deployment berdasarkan secret dan konfigurasi
workflow.

Untuk backend pada network Docker yang sama, `INTERNAL_API_URL` dapat memakai
nama service backend, misalnya `http://tracklume-api-api-1:8080/api/v1`.
Jangan memakai `localhost` dari container web karena akan menunjuk ke container
web itu sendiri.
